<template>
    <div id="chat_detail" ref="chat_detail"  @contextmenu.prevent.stop>
      <ScrollBar @pullLoading="pullLoading" @hideContextMenue="isShow = false" :dropHeight="dropHeigth" @dropLoading="dropLoading" :pulling="isPulling" :isBottom='isBottom' :isContextMenu="isShow" ref="scrollCont">
    ...
    calcLastHeight() {
      this.$nextTick(() => {
        if (this.$refs.msgWrapper && this.$refs.msgWrapper.length > 0) {
          ...
          this.dropHeigth = msgWrapper.clientHeight + 5
          if (this.isdropHeight) {
            this.showTips = false
          } else {
            this.showTips = true
